...
const Users = () => {
    ...
    function handleAdd() { setForm(true); }
        ...
        switch (type) {
            ...
            case "delete":
                confirmAlert({
                    ...
                    buttons: [
                        { label: 'Тийм', onClick: () => ConfirmDelete(clicked, 'user') },
    ...
    useEffect(() => {
        async function fetchUsers() {
            const data = await apiPostJSON('/operator', { action: 'fetch_user' });
        ...
        <div className="table-container">
            {role === 3 && (
                <div style={{ textAlign: 'end', marginRight: '2%' }}>
                    <button onClick={handleAdd} className="button add">Хэрэглэгч нэмэх</button>
                </div>
            )}
            <Modal show={state} handleClose={handleClose} nodeID={clicked}>
                <ul style={{ margin: '0', padding: '0' }}>
                    <li onClick={() => clickedMenu('edit')} className="menuItem">Засах</li>
                    <li onClick={() => clickedMenu('delete')} className="menuItem">Устгах</li>
                </ul>
            </Modal>
            <UserForm show={form} handleClose={handleClose} />
            <table>
                <thead>
                    <tr>
                        <th>ID</th><th>Овог</th><th>Нэр</th>
                        <th>Төрөл</th><th>Утас</th><th>Цахим шуудан</th><th></th>
                    </tr>
                </thead>
                <tbody>
                    {userList.map((val, key) => (
                        <tr key={key}>
                            <td className="link" onClick={() => window.location.pathname = `/userInfo/${val.ajiltan_id}`}>{val.ajiltan_id}</td>
